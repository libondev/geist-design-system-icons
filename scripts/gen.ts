  ...
  const fileHandler = {
    vue() {
      ...
      for (const [name, content] of Object.entries(svgMap)) {
        ...
        const props = attributes?.split(BREAK_LINE_REGEX)
          .map((attr) => {
    ...
    react() {
      fileContent += `import { createElement, memo, type MemoExoticComponent, type SVGProps, type ReactSVGElement } from 'react'\n`
